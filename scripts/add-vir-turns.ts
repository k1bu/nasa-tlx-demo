...
async function addVirTurns() {
	try {
		...
		console.log('Adding VIR turn-by-turn data...');
		...
		allVirTurns.forEach((turn) => {
			console.log(
				`- Turn ${turn.turnNumber}: ${turn.turnName} (Difficulty: ${turn.difficulty}/10)`
			);
		});
	} catch (error) {
...
// Function to clean up duplicate turns
async function cleanupDuplicateTurns() {
	try {
		console.log('Cleaning up duplicate VIR turns...');
		...
		console.log(`Found ${allTurns.length} turns for VIR`);

		// Find duplicates by turnNumber
		const turnNumbers = allTurns.map((turn) => turn.turnNumber);
		...
		console.log(`Found duplicate turn numbers: ${uniqueDuplicates.join(', ')}`);

		// For each duplicate turn number, keep only the first one
		for (const turnNumber of uniqueDuplicates) {
			const duplicateTurns = allTurns.filter((turn) => turn.turnNumber === turnNumber);

			// Keep the first one, delete the rest
			for (let i = 1; i < duplicateTurns.length; i++) {
				await db.delete(schema.trackTurns).where(eq(schema.trackTurns.id, duplicateTurns[i].id));

				console.log(`🗑️ Deleted duplicate: Turn ${turnNumber} - ${duplicateTurns[i].turnName}`);
			}
		}

		console.log('✅ Duplicate cleanup completed!');

		// Show final turn count
